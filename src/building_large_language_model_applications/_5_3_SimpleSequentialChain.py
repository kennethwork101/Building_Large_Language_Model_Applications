...
template2 = "You are translator. Given a text input, translate it to {language} Translation:"
...
def main(options):
    ...
    printit(f"llmchain: invoke {question}", response)
    printit(f"prompt1", prompt1)
    printit(f"prompt2", prompt2)
    printit(f"chain1", chain1)
    printit(f"chain2", chain2)
    printit(f"overall_chain", overall_chain)
    return response
...
def Options():
    ...
    parser.add_argument('--question', type=str, help='question', default='Cats and Dogs')
